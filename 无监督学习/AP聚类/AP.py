...
if __name__ == "__main__":
    ...
    print('对角线上的值Preference：', preference)
    ...
    for i, mul in enumerate(np.linspace(1, 4, 9)):
        ...
        af = model.fit(data)
        center_indices = af.cluster_centers_indices_ #获取聚类中心的位置
        n_clusters = len(center_indices) #获取质心个数
        print('p = {0:.2f}'.format(mul), p)
        print("聚类簇的个数为：", n_clusters)
        y_hat = af.labels_ #获取类标签
