...
if __name__ == "__main__":
    ...
    # 数据1的参数：(epsilon, min_sample) epsilon是聚类点半径(用于确定邻域大小)，min_sample是最小样本数
    #共设置六对参数
    ...
    for i in range(len(params)):
        eps, min_samples = params[i] #获取相应的参数值
        model = DBSCAN(eps=eps, min_samples=min_samples)
        model.fit(data)
        y_hat = model.labels_#获得相应的类标签

        core_indices = np.zeros_like(y_hat, dtype=bool)
        core_indices[model.core_sample_indices_] = True

        y_unique = np.unique(y_hat) #获取相应的聚类簇个数
        n_clusters = y_unique.size - (1 if -1 in y_hat else 0) #如果预测值为-1表示异常值，要从中减去
