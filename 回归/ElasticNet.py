...
def xss(y, y_hat):
    ...
    # 总体平方和, 被解释变量Y的观测值与其平均值的离差平方和（总平方和）(说明 Y 的总变动程度）
    tss = ((y - np.average(y)) ** 2).sum()

    # 残差平方和，在线性模型中衡量模型拟合程度的一个量
    rss = ((y_hat - y) ** 2).sum()
...
if __name__ == "__main__":
    ...
    for t in range(4):
        ...
        for i, d in enumerate(d_pool):
            ...
            output = '%s:%dOrder, params=' % (titles[t], d)
            if hasattr(lin, 'alpha_'):
                idx = output.find('params')
    ...
    plt.suptitle('Polynomial Linear Model', fontsize=22)
    plt.show()
    ...
    plt.xlabel('Experiment: Linear/Ridge/LASSO/Elastic Net', fontsize=15)
    plt.ylabel('XSS', fontsize=15)
    plt.title('Total TSS=?', fontsize=18)
    plt.grid(True)
    plt.show()
