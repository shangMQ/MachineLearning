...
if __name__ == "__main__":
    ...
    plt.xlabel("cost(million$)")
    plt.ylabel("sales(million$)")
    plt.title("Use three features fit the linear model: TV, Radio, and Newspaper")
    ...
    # 因为随机打乱，对测试集进行排序，方便图像显示效果
    order = y_test.argsort(axis=0)  # y_test是个列向量
    y_test = y_test.values[order]
    x1_test = x1_test.values[order, :]
    y_hat = linreg.predict(np.array(x1_test))
    mse = np.average((y_hat - np.array(y_test)) ** 2)  # Mean Squared Error
    rmse = np.sqrt(mse)  # Root Mean Squared Error
    print("(均方误差={:.2f}, 均方根误差={:.2f})".format(mse, rmse))
    ...
    plt.xlabel("cost(million$)")
    plt.ylabel("sales(million$)")
    plt.title("Use three features fit the linear model: TV, Radio, and Newspaper by sorted target")
    plt.legend(loc='upper right')
    plt.grid()
    plt.show()
