...
if __name__ == "__main__":
    ...
    #查准率：（tp）/(tp+fp)分类为真的样本中，真正是真值的样本所占比例
    ...
    # The best value is 1 and the worst value is 0.
    #查全率(召回率)：（tp）/(tp+fn) 原本为真值的样本中预测为真值的样本所占比例
    ...
    #手动计算f1值：print(2 * (precision * recall) / (precision + recall))
    ...
    for beta in np.logspace(-3, 3, num=7, base=10):
        ...
        #手动计算fβ值：print((1+beta**2)*precision*recall / (beta**2 * precision + recall))
